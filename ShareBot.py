    ...
    try:
        ...
        try:
            if (req.json()["status_code"] == 0):
                ...
                TotalSentShare += 1
                if DebugMode == True:
                    print(Colorate.Horizontal(Colors.white_to_green, f"Sent Share: {TotalSentShare}"))
                else:
                    print(Colorate.Horizontal(Colors.white_to_green, f"Sent Share: {TotalSentShare}"))
...
print(Colorate.Vertical(Colors.white_to_blue,(Center.XCenter(''' www.acode.uk / acode.uk / services.acode.uk / www.services.acode.uk
MAKE SURE YOUR LINK IS SIMILAR TO THIS OR YOU WILL GET A SYNTAX ERROR > https://vm.tiktok.com/                            
                                ''')), 1))


if (__name__ == "__main__"):
    itemID       = Write.Input("Input video link here: ", Colors.white_to_blue, interval=0.0001)
    amount       = Write.Input("Amount (0=inf) > ", Colors.white_to_blue, interval=0.0001)
    NThread      = Write.Input("Input displayed Thread Amount here:", Colors.white_to_blue, interval=0.0001)
